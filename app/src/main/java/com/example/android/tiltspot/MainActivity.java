...
public class MainActivity extends AppCompatActivity
    ...
    private class SensorTask extends AsyncTask<SensorEvent, Integer, ArrayList<Float>> {
        @Override
        protected ArrayList<Float> doInBackground(SensorEvent... sensorEvent) {
            int sensorType = sensorEvent[0].sensor.getType();
            ...
            switch (sensorType) {
                case Sensor.TYPE_ACCELEROMETER:
                    mAccelerometerData = sensorEvent[0].values.clone();
                    break;
                case Sensor.TYPE_MAGNETIC_FIELD:
                    mMagnetometerData = sensorEvent[0].values.clone();
                    break;
                default:
                    return null;
            }
            ...
            ArrayList<Float> orientationValues = new ArrayList<Float>();

            if (rotationOK) {
                SensorManager.getOrientation(rotationMatrixAdjusted, mValue);

                orientationValues.add(mValue[0]);
                orientationValues.add(mValue[1]);
                orientationValues.add(mValue[2]);
    ...
    private void updateUi(ArrayList<Float> orientationValues)
    {
        if (orientationValues == null)
            return;

        // Pull out the individual values from the array.
        float azimuth = orientationValues.get(0);
        float pitch = orientationValues.get(1);
        float roll = orientationValues.get(2);
